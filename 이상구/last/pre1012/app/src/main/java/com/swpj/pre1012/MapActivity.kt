...
////////////////////////
        ...
        fun findwaysnd(lNguser:Marker?):String { //주어진 거점과 가장 가까운 지점 지명을 반환
            ...
            rest = numtem[23] //가장 거리가 작은 값의 위치
        ...
        button1.setOnClickListener { //첫번째버튼 누를시
            ...
            marker.tag = "마커 1" // 마커 클릭을 위한 객체 생성
            marker.setOnClickListener { //생성된 마커 클릭시 리스너
                infoWindow.open(marker) //마커 클릭 시 정보창 띄움
                true

            }
            marker.setOnClickListener { //생성된 마커 클릭시 리스너2
                if (rest == 0) { //마커 클릭 시 rest(최종 위치가 저장된 변수)에 따라 fragment를 불러오는 부분
