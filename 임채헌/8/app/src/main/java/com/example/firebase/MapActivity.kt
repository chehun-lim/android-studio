...
class MapActivity : AppCompatActivity(),  OnMapReadyCallback {
    ...
    override fun onMapReady(naverMap: NaverMap) {
        ...
        fun destination(marker1: Marker?, marker2: Marker?, marker3: Marker?, marker4: Marker?) {
            if (marker1 != null && marker2 != null && marker3 != null && marker4 != null ) {
                ...
                val savepoint = findwaysnd(markers[3]) //최종 추천장소 이름
                Toast.makeText(
                    this, "${savepoint}",
                    Toast.LENGTH_SHORT
                ).show()
            ...
            }else if(marker1 != null && marker2 != null && marker3 == null && marker4 != null){
                val latitudes =
        ...
        button3.setOnClickListener {
            ...
            if (intent.hasExtra("Information")) {
                val info = intent.getSerializableExtra("Information") as Information
                if (info.name != null) {
                    database.child("rooms").child(info.roomName).child(num.toString()).child("latitude").setValue("0")
                    database.child("rooms").child(info.roomName).child(num.toString()).child("longitude").setValue("0")
                }
        ...
        button2.setOnClickListener {
            ...
            if (intent.hasExtra("Information")) {
                val info = intent.getSerializableExtra("Information") as Information
                if (info.name != null && clickMarker!=null ) {
                ...
                override fun onLocationResult(locationResult: LocationResult?) {
                    locationResult?.let {
                        ...
                        for((i, location) in it.locations.withIndex()) {
                            ...
                            markerx.position = LatLng(location.latitude, location.longitude)
    ...
    override fun onBackPressed() {
        ...
        if (intent.hasExtra("Information")) {
            val info = intent.getSerializableExtra("Information") as Information
            database.child("rooms").child(info.roomName).child(num.toString()).removeValue()
        }
